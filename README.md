...
## (Imaging Lyceum Only) (Optional) Building and backprojecting AirSAS data from scratch
...
Rendering the backprojection result (from the `arma_20k` directory):
```
...
Dan Brown (dcb19@arl.psu.edu) provided SVSS measurements and trajectories data for two tracks. The data and motion solution
were accessed as an intermediate result from PSU's beamformer (ASASIN). 

[The data can be downloaded here](https://drive.google.com/file/d/1499P-KSbiz9nBSRJXR6HAPQNQ71KnPOw/view?usp=sharing). Email me if you don't have access for some reason. 

### Request the Data
In accordance with funding agency guidelines: request access to the SVSS dataset by 
